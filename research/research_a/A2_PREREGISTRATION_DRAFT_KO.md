...
`fb4247528af568aaddd12c2e38b972cf84c7044d8992e92e17d3f541b5326f58`이다.
...
별도 Bonferroni family를 구성한다. Noise gate는 고정 probability-by-prefix grid 어디서든
joint SESOI-qualified advantage가 하나 이상일 것을 요구한다. 0.80에서 advantage를
요구하지 않는다. 이 level은 unique-mode boundary 근처의 degradation을 검정한다.
...
20,000 iteration을 수행했다. 135 worlds에서 all-width gate, any-noise gate,
conjunctive scope gate의 estimated power는 모두 1.0이었다. Noise 0.80 group power는
0.0이었으며 이를 명시적으로 보고한다. 이 결과를 숨기거나 required advantage로
바꾸지 않는다.
...
필수 순서는 이 초안의 external review, 전체 test 및 clean-room dry run, source freeze,
freeze digest의 commit과 공개 기록, 32-byte seed escrow 및 public commitment 생성, 해당
commitment의 commit과 push, one-shot execution이다. Seed commitment 이후 threshold,
endpoint 또는 cohort repair는 허용하지 않는다.
