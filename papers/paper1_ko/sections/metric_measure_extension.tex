...
finite-sample convergence theorem이 아니다.
...
그러한 항을 추가하면 별도의 discrepancy가 되며 식별가능성을 별도로 분석해야 한다.
...
\]
는 well-defined이다. 직접 합하면 $(X,Y)$와 $(Y,Z)$ marginal이 각각 원래
coupling임을 알 수 있다. $\pi_{XZ}$를 그 $(X,Z)$ marginal이라 하자.
$\gamma(x,y,z)>0$이면
