...
$\Pi_r(K,f)=(\Dgm_0(f),\ldots,\Dgm_d(f))$. This construction follows the
standard finite persistence framework
...
The following result is the standard stability theorem for persistence
diagrams and persistence modules
\cite{cohensteiner2007stability,chazal2009proximity}.
...
persistence modules in degree $k$. Because $K$ is finite, every vector space
in either module is finite-dimensional; in particular, the modules are
q-tame and admit interval decompositions and persistence diagrams
\cite{crawleyboevey2015decomposition}. The algebraic stability theorem
\cite{chazal2009proximity,bauerlesnick2015induced} gives a bottleneck distance no larger than
the interleaving distance. Hence $d_B(\Dgm_k(f),\Dgm_k(g))\leq\delta$.
