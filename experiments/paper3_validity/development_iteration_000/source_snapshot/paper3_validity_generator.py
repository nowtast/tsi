...
P3_VALIDITY_GENERATOR_ID = "P3-4B-NONCIRCULAR-TASK-GENERATOR-v1"
...
def goal_utility(
    ...
    goal_targets: tuple[int, int],
) -> int:
    """Return the exogenous task utility; no TSI discrepancy enters this rule."""

    values = code.as_tuple()
    primary, secondary = goal_layers
    return (
        2 * int(values[primary] == goal_targets[0])
        + int(values[secondary] == goal_targets[1])
    )
...
class DownstreamTaskSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            not 0 <= target < LAYER_CARDINALITIES[layer]
            for layer, target in zip(
                self.goal_layers,
                self.goal_targets,
                strict=True,
            )
        ):
            raise ValueError("task goal target is outside its layer")
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "goal_targets": list(self.goal_targets),
...
def _ranked_goal_candidates(
    ...
    pairs = tuple(combinations(range(len(LAYER_ORDER)), 2))
    candidates = tuple(
        (pair, reference)
        for pair in pairs
        for reference in range(PLAN_CANDIDATE_COUNT)
    )
    return tuple(
        sorted(
            candidates,
...
def _build_task(
    ...
    for goal_layers, reference in _ranked_goal_candidates(
        key,
        world_index,
        unit_index,
        task_index,
    ):
        reference_values = endpoints[reference].as_tuple()
        goal_targets = (
            reference_values[goal_layers[0]],
            reference_values[goal_layers[1]],
        )
        utilities = tuple(
            goal_utility(endpoint, goal_layers, goal_targets) for endpoint in endpoints
        )
        eligible = tuple(
            (left, right)
        ...
        if eligible:
            selected_goal = goal_layers, goal_targets, utilities, eligible
            break
    if selected_goal is None:
        raise RuntimeError("downstream task has no non-tied oracle plan pair")
    goal_layers, goal_targets, utilities, eligible = selected_goal
    left, right = min(
        eligible,
