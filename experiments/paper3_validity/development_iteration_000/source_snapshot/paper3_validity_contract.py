...
P3_VALIDITY_CONTRACT_ID = "P3-4B-VALIDITY-v1"
...
NONNEGOTIABLE_POLICIES = (
    "Downstream outcomes are defined only by exogenous goal utility and realized plan regret.",
    ...
    "The generic baseline includes training loss, raw latent errors, endpoint exactness, task difficulty, and model identity.",
    "The TSI model differs from the generic baseline only by prespecified I0 and fixed-layer discrepancy predictors.",
...
class ValidityStatisticalPlan:
    ...
    prediction_models: str = (
        "development_fitted_frozen_ridge_logistic_and_discrete_hazard"
    )
...
def audit_validity_contract() -> dict[str, object]:
    ...
    if UNITS_PER_WORLD != 16 or PROBE_HORIZON != 8:
