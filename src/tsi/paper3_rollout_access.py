"""One-shot seed escrow and hash-chained access ledger for P3-4A."""
...
from .paper3_rollout_contract import (
    P3_ROLLOUT_CONTRACT_ID,
    rollout_contract_digest,
)


P3_ROLLOUT_ACCESS_ID = "P3-4A-ROLLOUT-SEALED-ACCESS-v1"
COMMITMENT_FILENAME = "rollout_seed_commitment.json"
ESCROW_FILENAME = "rollout_seed.escrow"
LEDGER_FILENAME = "rollout_access_ledger.jsonl"
SECRET_BYTES = 32
ZERO_HASH = "0" * 64
EVENT_SEQUENCE = (
    "seed_commitment_created",
    "rollout_seed_revealed",
...
def initialize_rollout_seed(root: Path) -> None:
    root = Path(root)
    ...
    if all(existing):
        return
    if any(existing):
        raise RuntimeError("rollout sealed material is only partially initialized")

    secret = secrets.token_bytes(SECRET_BYTES)
    commitment = sha256(secret).hexdigest()
    unsigned = {
        "identifier": P3_ROLLOUT_ACCESS_ID,
        "parent_contract": P3_ROLLOUT_CONTRACT_ID,
        "parent_contract_digest": rollout_contract_digest(),
        "algorithm": "sha256",
    ...
    try:
        ...
        if written != len(secret):
            raise OSError("failed to write complete rollout seed")
...
def append_rollout_access_event(
    root: Path,
    event: str,
    metadata: Mapping[str, object],
) -> str:
    if event not in ALLOWED_APPEND_EVENTS:
        raise ValueError("unknown rollout access event")
    root = Path(root)
    ledger_path = root / LEDGER_FILENAME
    events = _read_events(ledger_path)
    if not events:
        raise RuntimeError("rollout access ledger is empty")
    expected_event = EVENT_SEQUENCE[len(events)] if len(events) < 6 else None
    if event != expected_event:
        raise RuntimeError(
            f"expected rollout event {expected_event!r}, received {event!r}"
        )
    reveal_count = sum(item.get("event") == "rollout_seed_revealed" for item in events)
    result_count = sum(
        item.get("event") == "rollout_result_evaluated" for item in events
    )
    reveal_count += event == "rollout_seed_revealed"
    result_count += event == "rollout_result_evaluated"
    payload = {
        "sequence": len(events),
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "event": event,
...
def audit_rollout_access(
    ...
    *,
    ...
    try:
        ...
        descriptor = {}
        errors.append(f"cannot parse rollout commitment: {error}")
    unsigned = {
        key: value for key, value in descriptor.items() if key != "descriptor_digest"
    }
    if descriptor.get("descriptor_digest") != _digest(unsigned):
        errors.append("rollout commitment descriptor digest mismatch")
    if descriptor.get("identifier") != P3_ROLLOUT_ACCESS_ID:
        errors.append("rollout access identifier changed")
    if descriptor.get("parent_contract_digest") != rollout_contract_digest():
        errors.append("rollout access parent contract digest changed")
    commitment = descriptor.get("commitment")
    if not isinstance(commitment, str) or len(commitment) != 64:
        errors.append("rollout commitment is not a SHA-256 digest")

    try:
        events = _read_events(ledger_path)
    except (OSError, json.JSONDecodeError) as error:
        events = []
        errors.append(f"cannot parse rollout access ledger: {error}")
    ...
    for sequence, event in enumerate(events):
        ...
        if event.get("sequence") != sequence:
            errors.append(f"rollout event {sequence} sequence mismatch")
        if event.get("previous_event_hash") != previous_hash:
            errors.append(f"rollout event {sequence} breaks the hash chain")
        if event.get("event_hash") != expected_hash:
            errors.append(f"rollout event {sequence} hash mismatch")
        reveal_count += event.get("event") == "rollout_seed_revealed"
        result_count += event.get("event") == "rollout_result_evaluated"
        if event.get("seed_reveals_after_event") != reveal_count:
            errors.append(f"rollout event {sequence} reveal count mismatch")
        if event.get("result_evaluations_after_event") != result_count:
            errors.append(f"rollout event {sequence} result count mismatch")
    ...
    if event_names != expected_names:
        errors.append("rollout access event sequence does not match its phase")
    ...
    if reveal_count != expected_reveals:
        errors.append("rollout seed reveal count is invalid")
    if result_count != expected_results:
        errors.append("rollout result evaluation count is invalid")
    expected_revealed = expected_phase == "final"
    if descriptor.get("revealed") is not expected_revealed:
        errors.append("rollout commitment reveal flag is invalid")
    ...
    try:
        ...
        errors.append(f"cannot stat rollout escrow: {error}")
    else:
        escrow_mode = stat.S_IMODE(escrow_stat.st_mode)
        escrow_size = escrow_stat.st_size
        if escrow_mode != 0:
            errors.append("rollout escrow must have mode 000")
        if escrow_size != SECRET_BYTES:
            errors.append("rollout escrow has the wrong byte length")
    payload = {
        "identifier": P3_ROLLOUT_ACCESS_ID,
...
def reveal_rollout_seed(
    ...
    *,
    ...
    if not audit["passed"]:
        raise RuntimeError("rollout seed is not in a valid zero-access state")
    commitment_path = root / COMMITMENT_FILENAME
    escrow_path = root / ESCROW_FILENAME
    descriptor = json.loads(commitment_path.read_text(encoding="utf-8"))
    ...
    if len(secret) != SECRET_BYTES or sha256(secret).hexdigest() != commitment:
        raise RuntimeError("revealed rollout seed does not match its commitment")
