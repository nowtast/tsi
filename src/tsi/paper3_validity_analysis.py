...
P3_VALIDITY_CONFIRMATORY_ANALYSIS_ID = "P3-4B-CONFIRMATORY-ANALYSIS-v2"
...
def _secondary_summary(
    ...
    primary = [
        row for row in scored if row.get("model") in PRIMARY_PREDICTIVE_MODELS
    ]
    ...
    first_failure = Counter(int(row["first_failure_time"]) for row in primary)
    return {
        "primary_population": {
            "models": list(PRIMARY_PREDICTIVE_MODELS),
            "record_count": len(primary),
            "event_rate": float(
                np.mean([row["any_task_failure"] for row in primary])
            ),
            "binary_baseline_brier": float(
                np.mean([row["binary_baseline_brier"] for row in primary])
            ),
            "binary_tsi_brier": float(
                np.mean([row["binary_tsi_brier"] for row in primary])
            ),
            ...
            "layer_aware_binary_brier_improvement": float(
                np.mean(
                    [row["layer_aware_binary_brier_improvement"] for row in primary]
                )
            ),
...
def analyze_validity_confirmatory(
    ...
    exact_scored = [
        row for row in scored if row.get("model") not in PRIMARY_PREDICTIVE_MODELS
    ]
    ...
    primary_population_complete = len(primary_scored) == expected_primary_records
    event_rate = float(
        np.mean([row["any_task_failure"] for row in primary_scored])
    )
    event_balance = 0.05 <= event_rate <= 0.95
    exact_controls_structural_zero = bool(
        exact_scored
        and all(row["any_task_failure"] == 0 for row in exact_scored)
    )
    ...
    passed = bool(
        design_complete
        and primary_population_complete
        and outcomes_noncircular
        and event_balance
        and exact_controls_structural_zero
        and holm_passed
    ...
    payload: dict[str, object] = {
        ...
        "decision_requirements": {
            ...
            "primary_predictive_population_complete": primary_population_complete,
            "outcomes_noncircular_and_probe_task_separated": outcomes_noncircular,
            "sealed_event_balance_passed": event_balance,
            "exact_controls_remained_structural_zero": exact_controls_structural_zero,
        ...
        "sealed_primary_event_rate": event_rate,
        "primary_predictive_models": list(PRIMARY_PREDICTIVE_MODELS),
        "layer_aware_sensitivity_is_non_gating": True,
        "estimand_scope": (
            "held-out same-task incremental criterion validity of structural "
            "discrepancy for model-selected plan regret"
        ),
        "oracle_endpoint_diagnostics_used": True,
        "temporal_prognostic_validity_supported": False,
